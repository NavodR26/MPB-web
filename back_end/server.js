...
// // ✅ Middleware
// app.use(cors({ origin: "*" })); // Allow all origins for testing (change to specific domain if needed)
// app.use(fileUpload()); // Enable file uploads
// app.use(express.json()); // Enable JSON request handling

// // ✅ Serve static files (for uploaded market reports)
// app.use("/market-reports", express.static(path.join(__dirname, "..", "public", "market-reports", "2025")));
...
// // ✅ POST Route for Adding Auction Dates
// app.post("/api/auction-dates", (req, res) => {
//     const auctions = readAuctions();
//     const newAuction = req.body;
...
//     newAuction.id = Date.now(); // Generate unique ID
//     auctions.push(newAuction);
//     writeAuctions(auctions);
...
// // ✅ GET Route for Fetching Auction Dates
// app.get("/api/auction-dates", (req, res) => {
//     res.json(readAuctions());
// });

// // ✅ DELETE Route for Removing an Auction Date
// app.delete("/api/auction-dates/:id", (req, res) => {
//     let auctions = readAuctions();
//     const { id } = req.params;
...
// ✅ Root Route
app.get("/", (req, res) => {
    res.send("✅ Server is running! Use /api/auction-dates to fetch or update auction dates.");
});
